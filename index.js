...
statbot.hears("status", ["status"], (text, reply) => {
  ...
  pm2.list((err, procs) => {
    if(err)
      ...
      console.log(procs.map(proc => proc.name + " up " + proc.pm2_env.pm_uptime).join('\n'));
  });
});
